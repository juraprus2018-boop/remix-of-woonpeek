  ...
  for (const rssUrl of RSS_URLS) {
    try {
      const res = await fetch(rssUrl, {
        headers: { "User-Agent": "Woonaanbod NL-BlogBot/1.0" },
      });
      if (!res.ok) continue;
...
const SYSTEM_PROMPT = `Je bent een ervaren Nederlandse journalist en woningmarkt-redacteur die al 12 jaar schrijft voor vakbladen en consumentenmedia. Je hebt een eigen column op Woonaanbod NL.nl.
...
Zorg dat het artikel actueel aanvoelt, praktische tips bevat, en relevant is voor mensen die actief op zoek zijn naar een woning in Nederland. Gebruik concrete voorbeelden en cijfers waar mogelijk. Gebruik GEEN em-dashes.`;
    ...
    try {
      ...
      const siteUrl = "https://www.woonaanbod-nl.nl";
      ...
      if (PAGE_ACCESS_TOKEN) {
        ...
        if (PAGE_ID) {
          ...
          const baseTags = ["#woningmarkt", "#woonaanbod-nl", "#vastgoed", "#Nederland"];
