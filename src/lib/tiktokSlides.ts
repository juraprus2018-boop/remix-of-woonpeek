...
function drawWatermark(ctx: CanvasRenderingContext2D) {
  ...
  ctx.textAlign = "right";
  ctx.fillText("Huurbaasje.nl", W - 60, H - 60);
  ctx.textAlign = "left";
}
...
async function slideStats(p: SlideProperty): Promise<Blob> {
  ...
  ctx.textAlign = "center";
  ctx.fillText("Bekijk op Huurbaasje.nl", W / 2, H - 195);
...
export async function downloadSlidesZip(p: SlideProperty): Promise<void> {
  ...
  a.href = url;
  a.download = `huurbaasje-tiktok-${p.slug || p.id}.zip`;
...
export function buildTikTokCaption(p: SlideProperty): string {
  ...
  const lines = [
    ...
    "",
    "👉 Volledige info & reageren via Huurbaasje.nl (link in bio)",
    "",
    buildHashtags(p),
  ];
  return lines.join("\n");
